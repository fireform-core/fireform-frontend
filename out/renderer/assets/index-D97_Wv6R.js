...
function loadLastOutputPath() {
  return localStorage.getItem(LAST_OUTPUT_KEY);
}
function saveLastOutputPath(path) {
...
const useStore = create((set, get) => ({
  ...
  setTemplates: (templates) => {
    ...
    const liveIds = new Set(templates.map((t2) => t2.id));
    const selectedFillIds = get().selectedFillIds.filter((id2) => liveIds.has(id2));
  ...
  upsertTemplate: (template) => {
    ...
    const index = templates.findIndex((t2) => t2.id === normalized.id);
    const updated = index >= 0 ? templates.map((t2, i) => i === index ? normalized : t2) : [normalized, ...templates];
    saveTemplates(updated);
    set({ templates: updated });
  },
  toggleFillSelection: (id2) => {
    const current = get().selectedFillIds;
    set({
      selectedFillIds: current.includes(id2) ? current.filter((i) => i !== id2) : [...current, id2]
    });
  },
  addFillSelection: (id2) => {
    const current = get().selectedFillIds;
    if (!current.includes(id2)) set({ selectedFillIds: [...current, id2] });
  },
...
const TYPE_VALUE_TO_LABEL = Object.fromEntries(
  FIELD_TYPES.map((t2) => [t2.value, t2.label])
);
...
async function uploadTemplatePdf(file, directory = DEFAULT_TEMPLATE_DIRECTORY) {
...
async function makeFillable(pdfPath) {
...
async function fillTemplate(payload) {
...
function normalizeFieldType(value) {
  return FIELD_TYPES.some((t2) => t2.value === value) ? value : "string";
}
function useFieldRows() {
  const [fieldRows, setFieldRows] = reactExports.useState([{ name: "", type: "string" }]);
  const addRow = reactExports.useCallback(() => {
    setFieldRows((prev) => [...prev, { name: "", type: "string" }]);
  }, []);
  const removeRow = reactExports.useCallback((index) => {
    setFieldRows((prev) => prev.filter((_, i) => i !== index));
  }, []);
  const updateRow = reactExports.useCallback((index, updates) => {
    setFieldRows(
      (prev) => prev.map((row, i) => i === index ? { ...row, ...updates } : row)
    );
  }, []);
  const moveRow = reactExports.useCallback((fromIndex, toIndex) => {
    setFieldRows((prev) => {
      const updated = [...prev];
  ...
  const seedFromApiFields = reactExports.useCallback(
    (fields) => {
      const hasExisting = fieldRows.some((r2) => r2.name.trim());
      if (hasExisting) {
...
Replace your current form fields with them? Your existing entries will be lost.`
      ...
      setFieldRows(
        fields.map((f2) => ({
          name: f2.description || f2.name || "",
          type: normalizeFieldType(f2.type)
        }))
      );
      return "seeded";
    },
  ...
  const resetRows = reactExports.useCallback(() => {
    setFieldRows([{ name: "", type: "string" }]);
  }, []);
  return { fieldRows, addRow, removeRow, updateRow, moveRow, seedFromApiFields, resetRows };
}
function FieldsBuilder({ rows, onUpdate, onRemove, onMove }) {
  const dragSourceRef = reactExports.useRef(null);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fields-builder", "aria-live": "polite", children: rows.map((row, index) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "field-drag-handle", "aria-hidden": "true", children: "⋮⋮" }),
...
function TemplateBuilder() {
  ...
  const { fieldRows, addRow, removeRow, updateRow, moveRow, seedFromApiFields } = useFieldRows();
  const [templateName, setTemplateName] = reactExports.useState("");
  const [selectedFile, setSelectedFile] = reactExports.useState(null);
  const [uploadedPath, setUploadedPath] = reactExports.useState(null);
  const [uploadedFieldCount, setUploadedFieldCount] = reactExports.useState(null);
  const [isDragActive, setIsDragActive] = reactExports.useState(false);
  const [makeFillableLoading, setMakeFillableLoading] = reactExports.useState(false);
  const [showHelp, setShowHelp] = reactExports.useState(false);
  const [status, setStatus] = reactExports.useState({ message: "", type: "" });
  const [jsonResponse, setJsonResponse] = reactExports.useState(null);
  const fileInputRef = reactExports.useRef(null);
  const addFieldBtnRef = reactExports.useRef(null);
  const currentFileRef = reactExports.useRef(null);
  ...
  async function handleMakeFillable() {
    ...
    try {
      ...
      if (!pdfPath) {
        ...
        pdfPath = upload["pdf_path"] || null;
        setUploadedPath(pdfPath);
      }
      const body = await makeFillable(pdfPath);
      const newPath = body["pdf_path"] || pdfPath;
      const count = typeof body["field_count"] === "number" ? body["field_count"] : null;
      setUploadedPath(newPath);
      setUploadedFieldCount(count);
      setStatus({
        message: count !== null ? `Fillable PDF created — ${count} field${count === 1 ? "" : "s"} detected.` : "Fillable PDF created.",
        type: "success"
      });
    } catch (e) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (body["pdf_path"]) {
        setPreviewPath(body["pdf_path"]);
        saveLastOutputPath(body["pdf_path"]);
      }
      ...
      setStatus({ message: e2.message, type: "error" });
    }
  }
  const destinationPath = selectedFile ? `${DEFAULT_TEMPLATE_DIRECTORY}/${selectedFile.name}` : "";
  const fieldCountMatch = selectedFile && uploadedFieldCount !== null ? uploadedFieldCount === fieldRows.length ? "match" : "mismatch" : null;
  const fieldCountText = fieldCountMatch === "match" ? `PDF has ${pluralize(uploadedFieldCount, "fillable field")} — your ${pluralize(fieldRows.length, "row")} match.` : fieldCountMatch === "mismatch" ? `PDF has ${pluralize(uploadedFieldCount, "fillable field")} — you have ${pluralize(fieldRows.length, "row")}.` : "";
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "panel card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Create Template" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { className: "stacked-form", onSubmit: handleSubmit, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "templateName", children: "Template Name" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          id: "templateName",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: "Template PDF File" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          ref: fileInputRef,
      ...
      !selectedFile ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: "Drag and drop a PDF here" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "or click to select a file" })
          ]
        }
      ) : /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "helper", children: selectedFile ? `Selected: ${selectedFile.name} (${formatBytes(selectedFile.size)}) — destination: ${destinationPath}` : "No PDF selected." }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inline-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            children: makeFillableLoading ? "Working..." : uploadedFieldCount !== null ? "Re-detect fields" : "Make this PDF fillable"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            onClick: () => setShowHelp((v2) => !v2),
            children: "?"
          }
        )
      ] }),
      showHelp && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { id: "makeFillableHelp", className: "helper", children: "Use this only if your PDF doesn't already have fillable form fields. It runs an AI step to detect fields and add them (slower). Skip it if your PDF is already fillable." }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: "Form Fields" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "helper", children: "What information should be filled in? Add one row per field. Fields are filled into your PDF in the order shown — drag to reorder." }),
      fieldCountMatch && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "p",
        {
          className: `field-count-badge ${fieldCountMatch}`,
          "aria-live": "polite",
          children: fieldCountText
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        FieldsBuilder,
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "submit", children: "Create Template" })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `status${status.type ? ` ${status.type}` : ""}`, "aria-live": "polite", children: status.message }),
    jsonResponse != null && /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { className: "json-output", children: JSON.stringify(jsonResponse, null, 2) })
  ] });
}
function useSpeechRecording(onTranscribed) {
  const [sttState, setSttState] = reactExports.useState("idle");
  const [sttStatus, setSttStatus] = reactExports.useState("");
  ...
  const start = reactExports.useCallback(async () => {
    ...
    recorder.addEventListener("stop", async () => {
      ...
      try {
        ...
        setSttStatus(`Transcription failed: ${e.message}`);
      }
      setSttState("idle");
    });
    recorder.start();
...
function FillForm() {
  const { templates, selectedFillIds, toggleFillSelection, setActiveTab, setPreviewPath } = useStore((s) => ({
  ...
  const [status, setStatus] = reactExports.useState({ message: "", type: "" });
  const [jsonResponse, setJsonResponse] = reactExports.useState(null);
  const [selectionError, setSelectionError] = reactExports.useState(false);
  ...
  async function handleSubmit(e) {
    ...
    const selectedModel = model || void 0;
    setStatus({ message: `Filling ${pluralize(count, "form")}…`, type: "info" });
    const results = [];
    const errors = [];
    for (const id2 of selectedFillIds) {
      try {
        const result = await fillTemplate({ template_id: id2, input_text: inputText.trim(), model: selectedModel });
        results.push(result);
      } catch (e2) {
        const template = templates.find((t2) => t2.id === id2);
        const label = template?.name || `id ${id2}`;
    ...
    if (lastResult) {
      ...
      if (lastResult["output_pdf_path"]) {
        const path = lastResult["output_pdf_path"];
  ...
  function handleTilePreview(pdfPath) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "panel card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Fill Form" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("form", { className: "stacked-form", onSubmit: handleSubmit, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "inputText", children: "Incident Input Text" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `stt-controls${isRecording ? " is-recording" : ""}${isPaused ? " is-paused" : ""}`, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            onClick: start,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "stt-dot", "aria-hidden": "true" }),
              "Record"
            ]
          }
        ),
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            disabled: !isRecording && !isPaused,
            onClick: stop,
            children: "Stop & Transcribe"
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "stt-status", "aria-live": "polite", children: sttStatus })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "textarea",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { children: "Forms to fill" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `helper${selectionError ? " error" : ""}`, "aria-live": "polite", children: !count ? "Select one or more forms to fill." : `${pluralize(count, "form")} selected.` }),
      !templates.length ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "empty-state", children: "No templates yet — create one in the Create Template tab." }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "template-tiles", children: templates.map((template) => {
          ...
          "div",
          {
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "tile-body", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tile-title", children: template.name || "Untitled" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "tile-meta", children: pluralize(fieldCount, "field") })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "fillModel", children: "Extraction model" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          id: "fillModel",
          ...
          children: models.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: "", children: "(default model)" }) : models.map((m2) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: m2, children: m2 === defaultModel ? `${m2} (default)` : m2 }, m2))
        }
      ),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
          ...
          children: count > 1 ? `Fill ${count} Forms` : "Fill Form"
        }
      )
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `status${status.type ? ` ${status.type}` : ""}`, "aria-live": "polite", children: status.message }),
...
function TemplatesList() {
  ...
  function handlePreview(pdfPath) {
    setPreviewPath(pdfPath);
    setActiveTab("pdfPreviewer");
  }
  function handleUseFill(templateId, templateName) {
    addFillSelection(templateId);
    setActiveTab("fillForm");
  }
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "panel card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Templates" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "helper", children: "Templates are loaded from the database and refreshed automatically." }),
    !templates.length ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty-state", children: "No templates stored yet." }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "template-list", children: templates.map((template) => {
      const entries = Object.entries(template.fields || {});
      return /* @__PURE__ */ jsxRuntimeExports.jsxs("article", { className: "template-card", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "template-meta", children: [
          "pdf_path: ",
          template.pdf_path || ""
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "fields-table", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Field" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("th", { children: "Type" })
          ] }) }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: entries.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { children: /* @__PURE__ */ jsxRuntimeExports.jsx("td", { colSpan: 2, children: "No fields." }) }) : entries.map(([name, type]) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: name }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("td", { children: TYPE_VALUE_TO_LABEL[type] || "Text" })
          ] }, name)) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "card-actions", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: "Preview This Template"
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
...
function PdfPreviewer() {
  ...
  const [inputPath, setInputPath] = reactExports.useState(storePreviewPath || "");
  const [frameUrl, setFrameUrl] = reactExports.useState("");
  const [status, setStatus] = reactExports.useState({ message: "", type: "" });
  const activeObjectUrlRef = reactExports.useRef(null);
  const prevStorePathRef = reactExports.useRef(storePreviewPath);
  reactExports.useEffect(() => {
  ...
  async function triggerPreview(path) {
    ...
    if (url) {
      ...
      setStatus({
        message: likelyLocal ? `Could not preview "${raw}". It looks like a server-local path and may not be web-accessible.` : `Could not preview path.`,
        type: "error"
      });
    }
  }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "panel card", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "PDF Preview" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preview-controls", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "localPdfFile", children: "Preview Local PDF" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "input",
        {
          id: "localPdfFile",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "preview-controls", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("label", { htmlFor: "serverPdfPath", children: "Preview from Template or Output Path" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "inline-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: handlePreviewPath, children: "Preview Path" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: `status${status.type ? ` ${status.type}` : ""}`, "aria-live": "polite", children: status.message }),
    frameUrl && /* @__PURE__ */ jsxRuntimeExports.jsx(
      "iframe",
      {
        id: "pdfFrame",
        title: "PDF Preview",
...
function LoadingScreen() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "loading-screen", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Starting up..." }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Initializing FireForm backend" })
  ] });
}
const TABS = [
...
function App() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-shell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "app-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Desktop App" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "FireForm" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "subtitle", children: "Report once, file everywhere." })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("nav", { className: "tabs", "aria-label": "Primary sections", children: TABS.map((tab) => /* @__PURE__ */ jsxRuntimeExports.jsx(
      "button",
      {
        type: "button",
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { children: [
      activeTab === "createTemplate" && /* @__PURE__ */ jsxRuntimeExports.jsx(TemplateBuilder, {}),
      activeTab === "fillForm" && /* @__PURE__ */ jsxRuntimeExports.jsx(FillForm, {}),
      activeTab === "templatesList" && /* @__PURE__ */ jsxRuntimeExports.jsx(TemplatesList, {}),
      activeTab === "pdfPreviewer" && /* @__PURE__ */ jsxRuntimeExports.jsx(PdfPreviewer, {})
    ] })
  ] });
}
